...
1. Negative-Binomial (Poisson-Gamma model)
...
- Parameters: $\mu$, $\theta$
- variance: $\mu / \theta$
- Extension of the double exponential family [@efron_double_1986] defined by pmf
$$
...
$$
- Includes spacial cases [@sellers_com-poisson_2012] of Poison$(\lambda)$ when $\nu = 1$, geometric$(\lambda)$
- Model can be implemented using R package `COMPoissonReg`
...
$$
- Zero inflated framework extends to other distributions to further capture over-dispersion
